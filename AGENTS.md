...
<!-- Last reviewed: July 16, 2026 — keep this date current at the end of every session -->
...
- `CONTEXTV29.md` — latest handoff: Approvals page loads are read-only; removed render-time orphan reconciliation that silently rejected pending outputs (July 16, 2026).
- `SESSION_2026-07-16.md` — July 16 critical bug investigation and fix (commit 22c3c13).
- `CONTEXTV28.md` — prior handoff: full codebase audit — `resolveClerkProfile` enforced at all 45 call sites, 16 dead API routes deleted, proxy/env cleanup, pre-push guard created; open items incl. unlinked `/dashboard/ai-toolkit` (July 13, 2026).
