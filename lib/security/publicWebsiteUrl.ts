...
/**
 * Require a public http(s) URL before any server-side website fetch.
...
): Promise<PublicWebsiteUrlResult> {
...
export async function assertPublicWebsiteUrl(raw: string | null | undefined): Promise<string> {
  const result = await validatePublicWebsiteUrl(raw)
  if (!result.ok) throw new UnsafeWebsiteUrlError(result.reason)
  return result.url
}
